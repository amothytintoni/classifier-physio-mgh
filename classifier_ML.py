...
for filename in list_file:
    ...
    #                                                                                       'skin_temperature',
    #                                                                                       'timeLengthValidity',
    #                                                                                       'RR_DC', 'RR_fdomain_w_good_sqa', 'RR_IBI',
    #                                                                                       'RR_TD',
    #                                                                                       'RR_tdomain', 'RR',
    #                                                                                       # 'sensor_onskin_status'
    #                                                                                       ]]
...
alternate_feature_names = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L', 'M']
alternate_feature_names_7 = ['A', 'B', 'C', 'D', 'E', 'F', 'G']
...
features = np.array(alternate_feature_names)
...
df_nodrop['activity_level'] = ((df_nodrop['point_awake']) /
                               (df_nodrop['point_sleep']+df_nodrop['point_awake']))
df_test['activity_level'] = ((df_test['point_awake']) /
                             (df_test['point_sleep']+df_test['point_awake']))
df_nodrop_test['activity_level'] = (
    (df_nodrop_test['point_awake'])/(df_nodrop_test['point_sleep']+df_nodrop_test['point_awake']))
# print(df.shape)
# activity_level
#############################
...
for i in range(0, trycount):
    ...
    clf_logit_all = sklm.LogisticRegression(
        C=2.0, penalty='l1', solver='saga', max_iter=3000, tol=1e-3).fit(df_4, pdv_1_vs_all)
    ...
    sens_logit_all_nodrop = cm_logit_all_nodrop[1, 1] / \
        (cm_logit_all_nodrop[1, 1] + cm_logit_all_nodrop[1, 0])
    spec_logit_all_nodrop = cm_logit_all_nodrop[0, 0] / \
        (cm_logit_all_nodrop[0, 0] + cm_logit_all_nodrop[0, 1])
    prec_logit_all_nodrop = cm_logit_all_nodrop[1, 1] / \
        (cm_logit_all_nodrop[1, 1] + cm_logit_all_nodrop[0, 1])

    cm_test_logit_all = confusion_matrix(pdv_1_vs_all_test, clf_logit_all.predict(df_test_4))
    sens_test_logit_all = cm_test_logit_all[1, 1] / \
        (cm_test_logit_all[1, 1] + cm_test_logit_all[1, 0])
    spec_test_logit_all = cm_test_logit_all[0, 0] / \
        (cm_test_logit_all[0, 0] + cm_test_logit_all[0, 1])
    prec_test_logit_all = cm_test_logit_all[1, 1] / \
        (cm_test_logit_all[1, 1] + cm_test_logit_all[0, 1])

    cm_test_logit_all_nodrop = confusion_matrix(
        pdvnd_1_vs_all_test, clf_logit_all.predict(df_test_4_nodrop))
    sens_test_logit_all_nodrop = cm_test_logit_all_nodrop[1, 1] / \
        (cm_test_logit_all_nodrop[1, 1] + cm_test_logit_all_nodrop[1, 0])
    spec_test_logit_all_nodrop = cm_test_logit_all_nodrop[0, 0] / \
        (cm_test_logit_all_nodrop[0, 0] + cm_test_logit_all_nodrop[0, 1])
    prec_test_logit_all_nodrop = cm_test_logit_all_nodrop[1, 1] / \
        (cm_test_logit_all_nodrop[1, 1] + cm_test_logit_all_nodrop[0, 1])
    ...
    fp_logit_all, tp_logit_all, threshold_logit_all = roc_curve(
        pdv_1_vs_all, clf_logit_all.predict_proba(df_4)[:, 1])
    area_logit_all = auc(fp_logit_all, tp_logit_all)

    logit_all_result = pd.DataFrame(
        data=[["logit all", sens_logit_all, spec_logit_all, prec_logit_all, area_logit_all]])
    ...
    clf_nn_all = MLPClassifier(activation='relu', hidden_layer_sizes=(
        600, 300), max_iter=3000, alpha=0.1, learning_rate_init=0.003, solver='adam', tol=1e-3).fit(df_4, pdv_1_vs_all)
    # ,early_stopping=True
    ...
    nn_all_result = pd.DataFrame(
        data=[["nn all", sens_nn_all, spec_nn_all, prec_nn_all, area_nn_all]])
    ...
    fp_test_logit_all, tp_test_logit_all, threshold_test_logit_all = roc_curve(
        pdv_1_vs_all_test, clf_logit_all.predict_proba(df_test_4)[:, 1])
    area_test_logit_all = auc(fp_test_logit_all, tp_test_logit_all)

    logit_all_result_test = pd.DataFrame(
        data=[["logit test all", sens_test_logit_all, spec_test_logit_all, prec_test_logit_all, area_test_logit_all]])
    ...
    fp_test_all, tp_test_all, threshold_test_nn_all = roc_curve(
        pdv_1_vs_all_test, clf_nn_all.predict_proba(df_test_4)[:, 1])
    area_test_nn_all = auc(fp_test_all, tp_test_all)

    nn_all_result_test = pd.DataFrame(
        data=[["nn test all", sens_test_nn_all, spec_test_nn_all, prec_test_nn_all, area_test_nn_all]])
...
# =============================================================================
    ...
    clf_svc_all = SVC(C=1, kernel='poly', tol=1e-3, max_iter=3000,
                      probability=True).fit(df_4, pdv_1_vs_all)
    ...
    fp_svc_all, tp_svc_all, threshold_svc_all = roc_curve(
        pdv_1_vs_all, clf_svc_all.predict_proba(df_4)[:, 1])
    area_svc_all = auc(fp_svc_all, tp_svc_all)

    svc_all_result = pd.DataFrame(
        data=[["svc all", sens_svc_all, spec_svc_all, prec_svc_all, area_svc_all]])

    fp_test_svc_all, tp_test_svc_all, threshold_test_svc_all = roc_curve(
        pdv_1_vs_all_test, clf_svc_all.predict_proba(all_test_reshaped_scaled)[:, 1])
    area_test_svc_all = auc(fp_test_svc_all, tp_test_svc_all)

    svc_all_result_test = pd.DataFrame(
        data=[["svc test all", sens_test_svc_all, spec_test_svc_all, prec_test_svc_all, area_test_svc_all]])
...
# =============================================================================
    ...
    plt.show()
#
    df_xgb_roc = pd.read_csv('roc_val.csv')
    fp_xgb = df_xgb_roc['fp']
    tp_xgb = df_xgb_roc['tp']

    plt.figure(8)
    plt.plot(fp_xgb, tp_xgb, 'r-', label='XGB, AUC = 0.961')
    plt.plot(fp_test_all, tp_test_all, 'g-', label='NN, AUC = %.3f' % area_test_nn_all)
    plt.plot(fp_test_svc_all, tp_test_svc_all, 'b-', label='SVC, AUC = %.3f' % area_test_svc_all)
    plt.plot(fp_test_logit_all, tp_test_logit_all, 'm-',
             label='Logit, AUC = %.3f' % area_test_logit_all)
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
    plt.title('Comparison of all ML technique\'s ROC')
    plt.legend()
    plt.show()
...
df_ML_results = df_ML_results.rename(
    columns={0: 'Features', 1: 'Sens', 2: 'Spec', 3: 'Prec', 4: 'ROC AUC'})
